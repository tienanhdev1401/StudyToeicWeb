...
const ManageStaff = () => {
  ...
  const [staffList, setStaffList] = useState([
    ...
    },
    {
      ...
      emailAddress: 'support1@example.com',
    ...
    },
    {
      ...
      emailAddress: 'content@example.com',
  ...
  // Filter staff (role not equal to 'USER')
  const filteredData = staffList
  ...
  return (
    <div className="staff-container">
        ...
        <tbody>
          {currentItems.map(item => (
            <tr key={item.id}>
              ...
              <td>{new Date(item.joinAt).toLocaleDateString()}</td>
              <td>
                <span className={`role-badge ${item.role.toLowerCase().replace('_', '-')}`}>
                  {item.role.replace('_', ' ')}
                </span>
              </td>
              <td>
      ...
      <DeleteConfirmModal
        ...
        onConfirm={() => {
          if (itemToDelete) {
            console.log("Deleting single staff:", itemToDelete);
            setStaffList(prev => prev.filter(staff => staff.id !== itemToDelete.id));
          } else {
            console.log("Deleting multiple staff:", selectedItems);
            setStaffList(prev => prev.filter(staff => !selectedItems.includes(staff.id)));
          }
